...
export class AppComponent  implements OnInit{
  ...
  isAnswerNotCorrect = false;
  correctAns="";
  hhh :Number=50;
  // stars: number[] = ["easy", "medium", "hard"];
  stars: String[] = ["easy","hard","medium"];
  allChoices =[];
  question ={};
  questionSixten="";
  //currentQuestion;
  Paper=[
    ...
    },
    {
      ...
      "type": "multiple",
      "difficulty": "hard",
      "question": "Which of these voices wasn t a choice for the House AI in The Simpsons Tree house  of Horror short House of Whacks?",
  ...
  ngOnInit(){
    this.totalQuestion=this.Paper.length;
    this.questionSixten=this.Paper[15].question
    this.correctAns = this.Paper[15].correct_answer
    this.question ={category:this.Paper[15].category,correctAnswer:this.Paper[15].correct_answer,difficulty:this.Paper[15].difficulty,incorrectAnswers:this.Paper[15].incorrect_answers,question:this.Paper[15].question,type:this.Paper[15].type} 
    console.log(this.Paper.length)
    this.Paper[15].incorrect_answers.push(this.Paper[15].correct_answer)
    this.correctAns = this.Paper[15].correct_answer
    this.allChoices =this.Paper[15].incorrect_answers
    this.selectedValue = 'hard'
    ...
    checkAnswer(event){
       if(event ===this.Paper[15].correct_answer){
        this.isAnswerCorrect= true 
        return  
       }
       else{
        this.isAnswerNotCorrect = true
        return
       }
    }
    checkProgressBar(){
      let percentage=16/this.Paper.length
    ...
    checkResult(){
      ...
      let incorrectAnsweres = this.Paper.length - correctAnswers
      let percentage=correctAnswers/currentQuestion
      percentage=percentage*100
    ...
    maxScore(){
      ...
      let percentage=correctAnswer/this.Paper.length;
      percentage=percentage*100;
        return percentage;
    }
    lowestScore(){
